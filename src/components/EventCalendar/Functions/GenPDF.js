...
export const getGeneratePDF = async ({
  ...
  const issuer =
    employees.allUser && employees.allUser.length > 0
      ? employees.allUser[0]
      : null;

  const issuerName = issuer
    ? `${issuer.fname} ${issuer.lname}`
    : "ไม่ทราบชื่อผู้ใช้";
  const issuerTel = issuer ? issuer.tel : "-";
  ...
  // 🔹 วาดข้อความทีละบรรทัด พร้อมตรวจสอบพื้นที่
  lines.forEach((line) => {
  ...
  doc.text(`ผู้ออกเอกสาร: ${issuerName}`, rightBoxX + 5, boxY + 10);
  doc.text(`เบอร์โทร: ${issuerTel}`, rightBoxX + 5, boxY + 20);
  doc.text(

...
const EventService = {
  ...
  /**
   * @param {object} [opts]
   * @param {boolean} [opts.slim] ขอเฉพาะฟิลด์ที่ใช้สรุปตัวเลข (ไม่เอาไฟล์แนบ/คอมเมนต์/ประวัติกิจกรรม)
   *   ⚠️ ใช้ได้เฉพาะหน้าที่ "ไม่แสดงรายละเอียดของงาน" เช่นหน้ารายงาน — หน้าการดำเนินงานต้องใช้ฟิลด์เต็ม
   *   ที่ 1000 งาน แบบเต็มราว 3.6 MB ต่อการเปิดหนึ่งครั้ง แบบ slim เหลือราว 1 ใน 10
   * @param {boolean} [opts.detail] ขอ "ประวัติกิจกรรม" (activityLog) มาด้วย
   *   ⚠️ ไม่ส่งตัวนี้ = ไม่ได้ activityLog — มันกินพื้นที่ 58.5% ของทั้งก้อนคนเดียว จึงไม่ส่งโดยปริยาย
   *   ⚠️ **หน้าที่บันทึกประวัติกิจกรรมต้องส่งเสมอ** เพราะเขียนแบบอ่านมาทั้งก้อน-ต่อท้าย-เขียนกลับ
   *      ถ้าได้ก้อนเปล่ามาแล้วบันทึก ประวัติทั้งชุดของงานนั้นจะหายไปเลย

...
/**
 * @param {"admin"|"technician"} role  ⚠️ นี่คือ "มุมมองของแจ้งเตือน" ไม่ใช่ role จริงของผู้ใช้ และ
 * ไม่ใช่การเช็คสิทธิ์ — "admin" = มุมหัวหน้า (เห็นงานใหม่/คำขอปิดงานของทุกคน) · "technician" =
 * มุมผู้ปฏิบัติงาน (เห็นเฉพาะผลที่เกิดกับงานตัวเอง) ผู้เรียกเป็นคนตัดสินด้วย can(...) แล้วส่งค่ามาให้
 * จึงจงใจไม่ import ตารางสิทธิ์เข้ามาที่นี่ (ดู Header.js / OperationBoard/index.js)
 */
  ...
  useEffect(() => {
    ...
    events.forEach((ev) => {
      ...
      if (firstRun) {
        ...
        // ✅ ฝ่ายขายไม่มีแจ้งเตือนจากปฏิทินของตัวเอง — นัดทุกอันเขาเป็นคนลงเอง ไม่มีใครมอบหมาย
        // ไม่ต้องรออนุมัติ และไม่มีคำขอปิดงาน
        // 🐛 ที่แก้: เดิมตกไปเข้าสาขาของช่าง ทำให้เซลลงนัดเองแล้วได้แจ้งเตือนกลับมาทันทีว่า
        // "คุณได้รับมอบหมายงานใหม่" ซึ่งไม่มีใครมอบหมายอะไรให้เลย
        // ⚠️ เรื่องที่เซลต้องรู้จริงๆ (ใบแจ้งงานถูกอนุมัติ/ตีกลับ) เป็นคนละคอลเลกชัน (Dispatch)
        // ส่งผ่าน push จาก server อยู่แล้ว ไม่ได้อาศัย hook นี้
      } else if (role === "admin") {
        ...
        if (!prev) {
          ...
          // shared/utils/approvalStatus.js) — แยกข้อความ/type ให้ชัดว่าเป็นงานที่ต้องอนุมัติ ไม่ใช่งานปกติ
